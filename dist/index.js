...
function loadEnvManager() {
    ...
    if (!raw) {
        process.stderr.write("错误: 环境变量 APOLLO_CONFIG_URLS 未设置。\n" +
            "请设置为一个 JSON 对象，例如：\n" +
    ...
    try {
        ...
        process.stderr.write(`错误: APOLLO_CONFIG_URLS 不是合法 JSON: ${err instanceof Error ? err.message : String(err)}\n`);
        process.exit(1);
    }
...
async function main() {
    ...
    process.stderr.write(`[apollo-config-mcp] 已启动，支持环境: ${envManager.listEnvs().join(", ")}\n`);
}
main().catch((error) => {
