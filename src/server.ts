...
export function createServer(envManager: EnvManager): McpServer {
  ...
  // env 参数 schema
  const envSchema = z.string().describe(
    `环境名（必填）。可用: ${envManager.listEnvs().join(", ")}`,
  );
  ...
  // 返回结果的辅助函数
  function notFound(env: string, appId: string, ns: string, key: string, cluster: string) {
    return {
      content: [
        {
          type: "text" as const,
  ...
  server.tool(
    ...
    async () => {
      ...
      return {
        content: [
          { type: "text" as const, text: JSON.stringify({ envs }, null, 2) },
        ],
      };
    },
  );
  ...
  server.tool(
    ...
    "读取 Apollo 原始配置内容",
    baseParams,
    async ({ env, appId, namespace, cluster }) => {
      const client = envManager.getClient(env);
      const ns = namespace ?? "application";
