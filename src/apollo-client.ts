import type { ApolloClientOptions, ApolloConfig } from "./types.js";
...
export class ApolloClient {
  ...
  private readonly defaultCluster: string;
  ...
  async getConfig(
    ...
    return (await response.json()) as ApolloConfig;
  }
  ...
  private async request(url: string, appId: string): Promise<Response> {
    ...
    try {
      ...
      throw new Error(
        `Apollo 请求失败: ${error instanceof Error ? error.message : String(error)}`,
      );
    }
